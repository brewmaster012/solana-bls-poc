...
pub mod solanabls {
    ...
    pub fn compute(ctx: Context<Compute>, n: u64) -> Result<()> {
        ...
        static SK_BYTES: [u8; 32] = [
            0x0c, 0x3b, 0x68, 0x7e, 0xa7, 0x1d, 0x49, 0x85, 0x84, 0xcb, 0x64, 0x01, 0x6a, 0x56,
            0x47, 0xcd, 0x95, 0x6f, 0xa1, 0x2c, 0xc0, 0x6c, 0x24, 0x01, 0xcf, 0xf1, 0xbc, 0xee,
            0xd8, 0x6f, 0xb0, 0xf6,
        ];
        let one = Fq::one();
        ...
        static pkx0: [u8; 32] = [
            0x12, 0x86, 0x3d, 0xe9, 0x9c, 0xc0, 0x03, 0x7a, 0xc6, 0xd0, 0x04, 0x0c, 0x76, 0xb7,
            0x07, 0x02, 0x73, 0x9e, 0x01, 0xd4, 0xd7, 0x38, 0x93, 0xed, 0x9f, 0xfc, 0xbf, 0x6a,
            0x6b, 0x01, 0x95, 0x0c,
        ];

        // "12790415566273137204438980474676206431551668194861774000289116208318672938344",
        static pkx1: [u8; 32] = [
            0x1c, 0x47, 0x1e, 0x60, 0xe1, 0xf7, 0x9d, 0xe8, 0x49, 0xd0, 0xb1, 0xe0, 0x0c, 0x3e,
            0xc2, 0xeb, 0x52, 0x5b, 0xd5, 0xa5, 0x38, 0x64, 0x70, 0x16, 0x3d, 0x0d, 0xcd, 0x83,
            0x28, 0x56, 0x25, 0x68,
        ];

        // "19990703290207056186001361313522144773565207870437521135189959374985874088302",
        static pky0: [u8; 32] = [
            0x2c, 0x32, 0x56, 0x4c, 0x01, 0x8b, 0x5e, 0x59, 0x86, 0x17, 0x56, 0x5d, 0xa8, 0x28,
            0x55, 0xa1, 0x43, 0x92, 0xa6, 0x01, 0x45, 0x1c, 0xbd, 0x12, 0xd9, 0x04, 0xc6, 0x02,
            0xa3, 0x9f, 0xa1, 0x6e,
        ];
        // "4358342334887501838847343362140356079440147357795966237968340896425437333463",
        static pky1: [u8; 32] = [
            0x09, 0xa2, 0xbb, 0xfd, 0xf4, 0x23, 0x20, 0x70, 0x14, 0x4a, 0x06, 0x04, 0xf2, 0x4a,
            0x15, 0xb2, 0xd0, 0xe5, 0xaa, 0x58, 0x35, 0x11, 0x53, 0xec, 0x22, 0xc4, 0x0c, 0x39,
            0x71, 0xa4, 0x87, 0xd7,
        ];
        // now comare the pairings: E(sig, pk) ==? E(hash, G2)
        let mut input2 = [0u8; 384];
        let hx = hash_point.x.into_bigint().to_bytes_be();
        let hy = hash_point.y.into_bigint().to_bytes_be();
        input2[0..64].copy_from_slice(&sig);
        ...
        input2[192..224].copy_from_slice(&hx);
        input2[224..256].copy_from_slice(&hy);
        ...
        // res is big endian 32 bytes; success if res[31] == 1 otherwise failed
        assert!(res[31] == 1, "signature verification failed");
        Ok(())
    }
}
